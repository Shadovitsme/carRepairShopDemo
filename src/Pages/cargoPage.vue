...
// TODO заполнить из бд пока тут условные значения
...
// country = categoryNames.country
// description = categoryNames.description
  ...
  <div class="w-full max-w-[1630px] mx-auto">
    ...
    <div class="w-full px-[18px] desktop:px-[118px]">
      ...
      <div class="desktop:h-[480px] mx-auto mb-8">
        <div class="tablet:flex h-full">
          <div v-if="countImg > 1"
            ...
            <button @click="bigImgNum = n" v-for="n in countImg" :key="n" :id="n"
              class="h-[145px] w-full border flex rounded-xl mb-2 border-gray-200">
              <img :id="n" class="my-auto w-full"
          ...
          <div
            ...
            <div v-if="countImg > 1" class="absolute w-full h-full flex justify-between p-1">
              <ArrowButton :left="true" v-if="countImg > 1"
                @click="bigImgNum > 1 ? bigImgNum-- : (bigImgNum = countImg)" class="my-auto"></ArrowButton>
              <ArrowButton @click="bigImgNum < countImg ? bigImgNum++ : (bigImgNum = 1)" v-if="countImg > 1"
                class="my-auto"></ArrowButton>
            </div>

            <img class="my-auto w-full"
              src="https://www.zapcar.ru/upload/iblock/c25/kcx3rbgrf701zkufbuxgt9a3rff31cbr/1646407335_11_kartinkin_net_p_avtozapchasti_kartinki_13.png" />
          </div>
          <div class="desktop:flex">
            <AboutCargo class="laptop:hidden" :manufacturer="getCargo(art).manufacturer" :name="getCargo(art).name"
              :characteristic="getCargo(art).characteristic" :colorArr="getCargo(art).color"></AboutCargo>
            ...
            <div
              ...
              <div v-if="getCargo(art).discount && getCargo(art).min_discount_quantity > counted"
              ...
              <cusotomButton v-if="toBusk" @click="addToBusket" color="blue" text="В корзину" class="w-full" />

              <div v-if="!toBusk" class="tablet:flex justify-between w-full">
                <RouterLink href="/busket">
                  <cusotomButton color="blue" text="Перейти в корзину" class="tablet:mr-2 w-full mb-3 tablet:mb-0" />
                </RouterLink>
            ...
            <!-- выбор цвета -->
            <AboutCargo class="hidden laptop:visible laptop:block" :manufacturer="getCargo(art).manufacturer"
              :name="getCargo(art).name" :characteristic="getCargo(art).characteristic" :colorArr="getCargo(art).color">
            </AboutCargo>
          </div>
        </div>
        <!-- priceCard -->
      ...
      <div class="desktop:h-[244px] mb-6">
        <div class="flex mb-5">
          <CheckboxButton :hug="true" :checked="switchChecked" @click="switchChecked = !switchChecked"
            class="mr-2 w-fit" :dark="true" text="Характеристики" id="char"></CheckboxButton>

          <CheckboxButton :hug="true" :checked="!switchChecked" @click="switchChecked = !switchChecked" :dark="true"
            class="w-fit" text="Описание" id="describtion"></CheckboxButton>
        </div>
        <div v-if="switchChecked">
          ...
          <div class="flex">
            ...
            <p v-for="item in categoryNames" :key="item.article" class="p1 text-gray-700 inline-block">
              {{ item.color }}
            </p>
          </div>
        </div>
